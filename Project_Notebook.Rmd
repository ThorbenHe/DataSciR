...
div(plot_ly(defTop, x=defTop$year, y = fct_rev(fct_inorder(defTop$name))) %>% layout(
  title = 'Years until forest area lost',xaxis = list(title = 'Years'),yaxis = list(title = 'Countries')
), align = "right")
```
...
div(plot_ly(defBot, x=defBot$year, y = fct_rev(fct_inorder(defBot$name)), color = "red") %>% layout(
  title = 'Years until forest area lost based on deforestation',xaxis = list(title = 'Years'),yaxis = list(title = 'Countries')
), align = "right")
```
...
trend_plot <- plot_ly(trend, x = ~AirPollution, y = ~factor(Country)) %>%
  ...
  layout(title = "Air Pollution in India and the rest of the World (1990-2020)",
         yaxis = list(title = "") , xaxis = list(title = expression(paste("Air pollution [", mu, "g/", m^3, "]"))))
