...
## [FAO. 2020. Global Forest Resources Assessment 2020](https://fra-data.fao.org/WO/fra2020/home/)  
...
temperature %>% select(Area, Element, Year, Value)
```
...
pm2_5 %>% select(Country, Variable, Year, Value)
```
...
ghg %>% mutate(`1990` = as.double(`1990`)) %>% select(Country, Sector, Gas, `1990`, `2018`)
```
...
forest_continents <- read_csv("FAOSTAT_data_continents.csv", col_types = cols())
...
```{r Load-Country-data}
forest_countries <- read_csv("FAOSTAT_data_6-9-2021.csv", col_types = cols())
...
The plot is showing an overview of the Forest Area for each continent in the period 1990-2020. One can see a decrease in the Americas and Africa while Asia sees a slight increase in forest area. The forest area in Europe seems to be quite stable.
...
**Findings**
* Russian federation has the largest forest cover in 2020
...
emissionData <- read_csv("historical_emissions.csv", col_types = cols())
...
forestArea <- read_csv("FAOSTAT_data_6-9-2021.csv", col_types = cols())
...
```{r forest-area-against-absorbed-carbon, echo=FALSE, warning=FALSE}
data <- read_csv("fra2020-carbonStock.csv", col_types = cols())
...
### TODO: which data set is used here?
