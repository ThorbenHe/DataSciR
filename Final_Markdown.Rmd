...
To begin with we want to give a general overview of global forest development over the last 30 years. Afterwards we want to dig in deeper into the topics of deforestation and reforestation by extracting the main driver countries, showing trends and investigating in possible correlations which is leading to a crucial prediction in how many years forest would be lost, if humankind continues to act as it has in the past. 
...
The data of the Global Forest Resources assessment (FAO) is containing 3 datasets. Data on forest development for the intervals between 1990 - 2020, which we mainly use to answer questions around reforestation and deforestation, as well as data on forest disturbance for the period 2000-2017 and data on total forest areas.
  1.1 Data on forest development:
```{r echo=FALSE, message=FALSE}
intervals <- tibble(read_csv("data/FRA_2020_data/Intervals_2021_05_05.csv"))
intervals %>% drop_na() %>% select(name, year, contains('1'))
```
  
```{r preprocessing_kiran, echo=FALSE, message=FALSE,warning=FALSE}
# TODO: Missing data set
Forest_Area<- tibble(read_csv("data/FRA_2020_data/fra2020-extentOfForest.csv"))
...
```{r echo=FALSE, message=FALSE}
fra_data_tmp <- tibble(read_csv("data/FRA_2020_data/Annual_2021_05_05.csv"))
...
```{r echo=FALSE, message=FALSE}
fra <- tibble(read_csv("data/FRA_2020_data/FRA_Years_2021_05_05.csv"))
fra %>% select(name, year, '7a_employment_tot', '4a_priv_own')
```

## [FAO. 2020. FAOSTAT Emissions-Land Use, Forest Land dataset.](http://www.fao.org/faostat/en/#data/GF)  
The next data set is also from the FAO and contains data on emissions and area change due to gain and losses of carbon stocks in living tree biomass for the period 1990-2020.
```{r echo=FALSE, message=FALSE}
emissions <- tibble(read_csv("data/Emissions_Land_Use_Forest_Land_E_All_Data_(Normalized)/Emissions_Land_Use_Forest_Land_E_All_Data_(Normalized).csv"))
...
3. Years not captured in the data set were replaced with the mean carbon stock per country\
...
6. Finally, the emission data set was merged with the carbon stock to perform correlation.

```{r carbon-stock-dataset, include=FALSE}
data <- tibble(read.csv("data/fra2020-carbonStock.csv"))
identical(data, emissions)
...
forestArea <- read_csv("data/FAOSTAT_data_6-9-2021.csv", col_types = cols())
...
```{r echo=FALSE, message=FALSE}
temperature_data_tmp <- tibble(read_csv("data/Environment_Temperature_change_E_All_Data_(Normalized)/Environment_Temperature_change_E_All_Data_(Normalized).csv"))
temperature_data_tmp2 <- tibble(read_csv("data/FAOSTAT_data_temp_change.csv"))
identical(temperature_data_tmp, temperature_data_tmp2)
temperature_data_tmp %>% select(Area, Element, Year, Value)
```
...
## [OECD. 2021. Air quality and health: Exposure to PM2.5 fine particles](https://stats.oecd.org/viewhtml.aspx?datasetcode=EXP_PM2_5&lang=en)  
...
```{r echo=FALSE, message=FALSE}
AirPollution <- tibble(read_csv("data/EXP_PM2_5_Air_Pollution.csv"))
...
## [CAIT data: Climate Watch. 2020. GHG Emissions](https://www.climatewatchdata.org/data-explorer/historical-emissions?historical-emissions-data-sources=cait&historical-emissions-gases=all-ghg&historical-emissions-regions=All%20Selected&historical-emissions-sectors=total-including-lucf&page=1)  
The CAIT dataset contains data on greenhouse gas (GHG) emissions for the period from 1990-2018.
```{r echo=FALSE, warning=FALSE}
emissionData <- read_csv("data/historical_emissions.csv", col_types = cols())
...
**Preprocessing**
The first data set containing data about the volume of carbon emitted per country per year (1990-2020) is processed here
...
forest_continents <- read_csv("data/FAOSTAT_data_continents.csv", col_types = cols())
...
```{r Load-Country-data}
forest_countries <- read_csv("data/FAOSTAT_data_6-9-2021.csv", col_types = cols())
...
The bar chart below shows trends in the deforestation over the last thirty years over 5 continents. Unfortunately, our data set only provides intervals in 5 respectively 10 year steps, which is why it is not possible to show a more precise trend line.
...
plot_ly(trends_def,x=~year,y=~t_deforestation,color = trends_def$continent) %>% layout(
    title = 'Deforestation Trends over last three decades',xaxis = list(title = 'Decades'),yaxis = list(title = 'Total Deforesstation [1000 ha]')
  )
```
...
```{r residual-plot-Air-Pollution, echo=FALSE, message=FALSE}
augment(fit$fit) %>%
...
That’s why we want to put the GHG emission in to relation with the forest area. The objective of our analysis is to investigate how much GHG will be absorbed if the forest area is increased. To do so, we use the carbon stock and forest area data sets already worked on, which we visualize below.

```{r forest-area-against-absorbed-carbon, echo=FALSE, warning=FALSE, message=FALSE}
data <- read_csv("data/fra2020-carbonStock.csv", col_types = cols())
forestArea <- read_csv("data/FAOSTAT_data_6-9-2021.csv", col_types = cols())
colnames(forestArea)[colnames(forestArea) == "Area"] <- "country"
colnames(forestArea)[colnames(forestArea) == "Value"] <- "Area"
...
The bottom line on what we've learned from our data is that forests change incredibly fast. Even in our given time range from the last 30 years we gained many fascinating findings. Starting with the forest area we detected that there are huge differences between countries and even continents.
...
[1] Field, Christopher B., and Vicente R. Barros, eds. Climate change 2014–Impacts, adaptation and vulnerability: Regional aspects. Cambridge University Press, 2014.

[2] Messier, Christian, Klaus J. Puettmann, and K. David Coates, eds. Managing forests as complex adaptive systems: building resilience to the challenge of global change. Routledge, 2013.

[3] FAO. 2020. Global Forest Resources Assessment 2020: Main report. Rome. https://doi.org/10.4060/ca9825en

[4] Hannah Ritchie and Max Roser (2021) - "Forests and Deforestation". Published online at OurWorldInData.org. Retrieved from: 'https://ourworldindata.org/forests-and-deforestation' [Online Resource]

[1] https://www.zeit.de/wissen/umwelt/2021-06/brasilien-abholzung-amazonasgebiet-regenwald-mai-hoechststand

[2] https://web.archive.org/web/20100222172301/http://www.tagesschau.de:80/ausland/sahelzone100.html.
