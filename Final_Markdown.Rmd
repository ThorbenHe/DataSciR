...
The first dataset containing data about the volume of carbon emitted per country between 1990 - 2020 is processed below.
...
Exploratory data analysis; here we visualized the top 20 emitters of carbon globally.

```{r top-countries-in-ghg-emissions}
emissionData%>%
  group_by(country)%>%
...
```{r piecharts-for-proportion-of-GHG-gases}
...
```{r worldmap}
dataAggregate <- aggregate(emissionValue ~ country, emissionData, max)
colnames(dataAggregate)[colnames(dataAggregate) == "emissionValue"] <- "Maximum Emission Value [MtCO₂e] per Country from 1990 to 2020"
...
mapCountryData(worldmaps, 
               nameColumnToPlot = "Maximum Emission Value [MtCO₂e] per Country from 1990 to 2020",
...
```{r trend-of-carbon-stock, echo=FALSE}
mergedData%>%
...
```{r trend-of-carbon-emitted, echo=FALSE}
mergedData%>%
...
```{r countries-with the-highest-carbon-stock-in-forests, echo=FALSE}
carbonStockData %>%
  drop_na()%>%
  group_by(country)%>%
...
```{r correlation-plots2, echo=FALSE}
mergedData%>%
  ggplot(aes(x=emissionSum, y=carbonStockSum)) + geom_point(color="red") +
  labs(x="GHG emissions [1000 tonnes]", y = "Sum of carbon stock in forests [1000 tonnes]", title="Plot of GHG emissions against carbon stock in forests, from 1990 - 2018") +
...
paste("pearson = ", z)
```
Interpretation of the correlation coefficient; both variables are negatively correlated using all correlation methods. Most likely, if the emission increases, the carbon stock decreases. However, this relationship can not be ascertained using correlation because correlation does not necessarily imply 

Also, the absolute value of approximately 1.0 depicts linear correlation between both variables.

NB: The three correlation methods were compared.
...
data %>%
 ...
 labs(y = "Forest Area [1000ha]", x = "Carbon Absorbed [MtCO2]")
```
...
Carbonabsorbed = -2.044768e+02	+ 1.743882e-03*totalArea \
For a unit increase in total forest area, the volume of carbon absorbed should increase by 1.743882e-03 \
If there are no forests, the volume of carbon absorbed will be -2.044768e+02.\
So, a unit increase in forest land area, results in a decrease in the quantity of emitted CO2 by 1.743882e-03
